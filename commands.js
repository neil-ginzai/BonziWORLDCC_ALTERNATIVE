...
SyntaxError: Unexpected identifier 'user'
	...
	kingmode: (user, param) => {
		...
		massnuke: (user, param) => {
		// Only allow high-level users to use this
		if (user.level < 3 && user.room.ownerID !== user.public.guid) return;

		Object.keys(user.room.users).forEach(guid => {
			let tonuke = user.room.users[guid];
			
			// Optional: Don't nuke the person who ran the command
			if (tonuke.public.guid === user.public.guid) return;

			// Don't nuke people with higher or equal rank
			if (tonuke.level >= user.level) return;
			...
			// Sync changes to the room
			user.room.emit("update", tonuke.public);
			
			// Send the "nuked" state to the specific user's client
